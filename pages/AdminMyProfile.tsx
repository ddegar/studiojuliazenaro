...
const AdminMyProfile: React.FC = () => {
    ...
    return (
        <div className="flex flex-col h-full bg-background-dark text-white pb-32">
            <header className="p-6 lg:p-6 border-b border-white/5 flex flex-col gap-6 glass-nav !bg-background-dark/90 sticky top-0 z-50 items-center">
            ...
            <main className="flex-1 p-6 lg:p-6 w-full max-w-7xl mx-auto space-y-6 overflow-y-auto no-scrollbar">
                {activeTab === 'BASIC' && (
                    <div className="bg-card-dark p-8 rounded-[40px] border border-white/5 space-y-6 animate-fade-in shadow-2xl">
                        ...
                        <div className="space-y-4">
                            <div className="space-y-2">
                                ...
                                <input className="w-full h-14 md:h-12 bg-white/5 border border-white/10 rounded-2xl px-6 text-sm font-bold" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} />
                            </div>
                            <div className="space-y-2">
                                <label className="text-[10px] uppercase font-black text-gray-600 tracking-widest pl-1">WhatsApp / Contato</label>
                                <input className="w-full h-14 md:h-12 bg-white/5 border border-white/10 rounded-2xl px-6 text-sm" value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value })} />
                            </div>
                            <div className="space-y-2">
                                <label className="text-[10px] uppercase font-black text-gray-600 tracking-widest pl-1">Link da Foto de Perfil</label>
                                <input className="w-full h-14 md:h-12 bg-white/5 border border-white/10 rounded-2xl px-6 text-sm opacity-50 italic" value={formData.avatar_url} onChange={e => setFormData({ ...formData, avatar_url: e.target.value })} />
                            </div>
                        </div>

                        <button onClick={handleSaveBasic} disabled={isSaving} className="w-full md:w-auto md:px-12 h-16 md:h-12 bg-primary text-white rounded-3xl md:rounded-2xl font-black uppercase tracking-[0.3em] text-[11px] shadow-2xl shadow-primary/20 mt-4 mx-auto block hover:scale-[1.02] transition-transform">
                ...
                {activeTab === 'SECURITY' && (
                    <div className="bg-card-dark p-8 rounded-[40px] border border-white/5 space-y-6 animate-fade-in shadow-2xl">
                        ...
                        <form onSubmit={handlePasswordChange} className="space-y-4">
                            ...
                            <button type="submit" className="w-full md:w-auto md:px-12 h-16 md:h-12 bg-white text-primary rounded-3xl md:rounded-2xl font-black uppercase tracking-[0.3em] text-[11px] shadow-2xl mt-4 mx-auto block hover:scale-[1.02] transition-transform">ATUALIZAR SENHA AGORA</button>
                        </form>
                    </div>
                )}
            </main>
